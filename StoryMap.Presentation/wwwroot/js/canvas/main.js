document.addEventListener("DOMContentLoaded", () => {
  ...
  let panStartX = null,
    panStartY = null; // Track start position for panning
  let canvasOffsetX = 0,
    canvasOffsetY = 0; // Canvas offset for panning
  ...
  canvas.addEventListener("wheel", (e) => {
    ...
    const minZoomForImages =
      Math.min(canvas.width / imgWidth, canvas.height / imgHeight) * 500;
  ...
  function drawCanvas() {
    ...
    for (const { img, x, y, width, height } of images) {
      ctx.drawImage(img, x / 1000, y / 1000, width / 1000, height / 1000); // Draw each image
    }
    ctx.restore();
  }
});
